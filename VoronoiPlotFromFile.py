...
def PullFrames(name, FrameNum, Frames, Balls):
    ...
    df = pd.read_csv('data_files/%s.csv' % name, sep = '\t', index_col = 0)
...
def PlotVoronois(name, **kwargs):
    ...
    for i in range(Frames):
        ...
        if not exists('voronoi_plots_%s' % name):
            makedirs('voronoi_plots_%s' % name)

        plt.savefig('voronoi_plots_%s/voronoi-%04d.png' % (name, FrameNum + i))
        plt.clf()
